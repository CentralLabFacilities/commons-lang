 ...
 * @version $Id$
 */
public abstract class EnumClass implements Comparable, Serializable {
    ...
    private void init(String name) {
        ...
        Class cls = getClass();
        boolean ok = false;
        while (cls != null && cls != EnumClass.class && cls != ValuedEnum.class) {
            if (cls == enumClass) {
                ok = true;
                break;
            }
        ...
        Entry entry;
        synchronized( EnumClass.class ) { // LANG-334
            // create entry
     ...
     *  is <code>null</code>
     */
    protected static EnumClass getEnum(Class enumClass, String name) {
        Entry entry = getEntry(enumClass);
        if (entry == null) {
            return null;
        }
        return (EnumClass) entry.map.get(name);
    }
    ...
    protected static Iterator iterator(Class enumClass) {
        return EnumClass.getEnumList(enumClass).iterator();
    }
    ...
    private static Entry getEntry(Class enumClass) {
        ...
        if (EnumClass.class.isAssignableFrom(enumClass) == false) {
    ...
    public final boolean equals(Object other) {
        if (other == this) {
            ...
            return iName.equals(((EnumClass) other).iName);
        } else {
    ...
    public int compareTo(Object other) {
        ...
        return iName.compareTo(((EnumClass) other).iName);
    }
